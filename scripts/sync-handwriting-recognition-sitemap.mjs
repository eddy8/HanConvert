...
const markerStart = "  <!-- handwriting-recognition-tool:start -->";
const markerEnd = "  <!-- handwriting-recognition-tool:end -->";
const locales = [
  { hreflang: "zh-Hans", prefix: "", priority: "0.9" },
  { hreflang: "zh-Hant", prefix: "zh-tw/", priority: "0.9" },
  { hreflang: "en", prefix: "en/", priority: "0.9" },
  { hreflang: "ja", prefix: "ja/", priority: "0.9" },
...
function buildUrl(locale) {
  const alternates = locales.map(
    (item) => `    <xhtml:link rel="alternate" hreflang="${item.hreflang}" href="${absolutePath(item.prefix)}" />`
  );
...
console.log("Added 5 handwriting-recognition URLs to sitemap.xml.");
